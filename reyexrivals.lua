...
local Theme = {
    OuterBorder     = Color3.fromRGB(180, 35, 35),
    BorderPink      = Color3.fromRGB(200, 45, 45),
    BorderPinkDark  = Color3.fromRGB(140, 25, 25),

    WindowBg        = Color3.fromRGB(18, 12, 12),
    WindowBgTop     = Color3.fromRGB(24, 14, 14),
    WindowBgBottom  = Color3.fromRGB(12, 8, 8),
    InnerCanvasBg   = Color3.fromRGB(16, 10, 10),
    HeaderBg        = Color3.fromRGB(22, 12, 12),

    CardBg          = Color3.fromRGB(28, 16, 16),
    CardBgTop       = Color3.fromRGB(38, 20, 20),
    CardBgBottom    = Color3.fromRGB(18, 10, 10),
    BorderDark      = Color3.fromRGB(55, 30, 30),
    BorderCard      = Color3.fromRGB(70, 35, 35),

    AccentPink      = Color3.fromRGB(220, 50, 50),
    AccentPinkLight = Color3.fromRGB(255, 80, 80),
    AccentPinkDark  = Color3.fromRGB(160, 30, 30),
    AccentPinkDim   = Color3.fromRGB(100, 20, 20),

    TextWhite       = Color3.fromRGB(245, 240, 240),
    TextMuted       = Color3.fromRGB(160, 130, 130),
    TextDark        = Color3.fromRGB(110, 85, 85),
    ControlBg       = Color3.fromRGB(12, 8, 8),
    ButtonBg        = Color3.fromRGB(30, 16, 16),
    ButtonHoverBg   = Color3.fromRGB(50, 25, 25),
    ButtonBorder    = Color3.fromRGB(75, 40, 40),
    Red             = Color3.fromRGB(235, 60, 60),
    Yellow          = Color3.fromRGB(245, 195, 65),

    AccentGreen     = Color3.fromRGB(220, 50, 50),
    AccentGreenLight= Color3.fromRGB(255, 80, 80),
    AccentGreenDark = Color3.fromRGB(160, 30, 30),
    AccentGreenDim  = Color3.fromRGB(100, 20, 20)
}
...
watermarkFrame.Name = "ReyexWatermark"
...
wmLbl.Font = MainFont
wmLbl.RichText = true
wmLbl.Text = '<b>R</b>  |  <font color="#dc3232">reyex.hub</font>  |  60 fps  |  0 ms'
...
local function initTargetVisualizer()
    ...
    hud.BackgroundColor3 = Color3.fromRGB(18, 12, 12)
    ...
    local stroke = Instance.new("UIStroke")
    stroke.Color = Color3.fromRGB(55, 30, 30)
    stroke.Thickness = 1.2
    stroke.Parent = hud

    local topLine = Instance.new("Frame")
    topLine.Size = UDim2.new(1, 0, 0, 2.5)
    topLine.BackgroundColor3 = Color3.fromRGB(220, 50, 50)
    ...
    avatar.BackgroundColor3 = Color3.fromRGB(22, 12, 12)
    ...
    avStroke.Color = Color3.fromRGB(220, 50, 50)
    ...
    hpBg.BackgroundColor3 = Color3.fromRGB(30, 16, 16)
    ...
    vFolder.Name = "ReyexTargetVisualizer"
    ...
    for i = 1, 40 do
        ...
        p.Color = Color3.fromRGB(220, 50, 50)
        p.Transparency = 1
    ...
    for i = 1, 30 do
        ...
        wingL.Color = Color3.fromRGB(220, 50, 50)
        ...
        wingR.Color = Color3.fromRGB(220, 50, 50)
...
local function setMenuVisible(visible)
    ...
    if visible then
        ...
        pcall(function()
            ContextActionService:UnbindAction("ReyexMenuFreeze")
        end)
    end
end
...
titleLbl.Text = '<font color="#ffffff">Reyex</font><font color="#dc3232"> Hub</font>'
...
statusLeft.Text = 'welcome back, <font color="#dc3232">' .. LocalPlayer.DisplayName .. '</font>'
...
statusRight.Text = '<font color="#dc3232">[</font> <font color="#888894">rivals</font> <font color="#dc3232">]</font>'
...
addButton(gbSessionActions, "Rejoin server", function()
    ShowNotification("Reyex Hub", "Reconnecting to experience...", "INFO", 3)
    task.spawn(function()
        task.wait(0.5)
        pcall(function()
...
addButton(gbSessionActions, "Join lowest-ping server", function()
    ShowNotification("Reyex Hub", "Searching for lowest-ping public server...", "INFO", 3)
    task.spawn(function()
        ...
        if targetServer then
            ShowNotification("Reyex Hub", string.format("Joining server (%d ms ping)...", targetServer.ping or 0), "SUCCESS", 3)
            task.wait(0.5)
            pcall(function()
                TeleportService:TeleportToPlaceInstance(placeId, targetServer.id, LocalPlayer)
            end)
        else
            ShowNotification("Reyex Hub", "Connecting to optimal regional server...", "INFO", 3)
            task.wait(0.5)
            pcall(function()
...
uiRegistry["UnlockAllSkins"] = addCheckbox(gbSkins, "Unlock all skins (Client)", Config.UnlockAllSkins, function(v)
    ...
    if v then
        ...
        ShowNotification("Reyex Hub", "All 1,249 skins & cosmetics unlocked client-side.", "SUCCESS", 3)
    end
end)
...
function ProfileSystem.readStore()
    local ok, res = pcall(function()
        if readfile and isfile and isfile("reyex_hub_config.json") then
            local raw = readfile("reyex_hub_config.json")
    ...
    if not (isfile and isfile("reyex_hub_config.json")) then
...
function ProfileSystem.writeStore()
    ...
    local ok = pcall(function()
        ...
        writefile("reyex_hub_config.json", json)
    end)
    return ok
end
...
function ProfileSystem.applyProfile(pName, notify)
    ...
    if not data then
        if notify then
            ShowNotification("Reyex Hub", "Profile not found: " .. pName, "WARN", 2.5)
        end
        return false
    end
    ...
    if notify then
        ShowNotification("Reyex Hub", "Loaded profile: " .. pName, "SUCCESS", 2.5)
    end
    return true
end
...
function ProfileSystem.saveProfile(pName)
    ...
    ShowNotification("Reyex Hub", "Saved profile: " .. cleanName, "SUCCESS", 2.5)
end
...
function ProfileSystem.setAutoload(pName)
    ...
    ShowNotification("Reyex Hub", "Auto-load set to: " .. target, "SUCCESS", 2.5)
end

function ProfileSystem.deleteProfile(pName)
    local target = pName or ProfileSystem.current
    if #ProfileSystem.names <= 1 then
        ShowNotification("Reyex Hub", "Cannot delete only remaining profile.", "WARN", 2.5)
        return
    end
    ...
    ShowNotification("Reyex Hub", "Deleted profile: " .. target, "INFO", 2.5)
end
...
uiRegistry["MobileToggle"] = addCheckbox(gbShortcuts, "Mobile toggle button", Config.MobileToggle, function(v)
    ...
    local mBtn = screenGui:FindFirstChild("ReyexMobileToggle")
...
local function createESPForPlayer(p)
    ...
    chamsHighlight.Name = "ReyexHighlight"
...
table.insert(activeConnections, RunService.RenderStepped:Connect(function(dt)
    ...
    if curTime - lastFpsSampleTime >= 0.25 then
        ...
        if wmLbl and wmLbl.Parent then
            wmLbl.Text = '<b>R</b>  |  <font color="#dc3232">reyex.hub</font>  |  ' .. tostring(liveFps) .. ' fps  |  ' .. tostring(livePing) .. ' ms'
        end
                                    ...
                                    if not hackerNotifiedTimestamps[p] or tick() - hackerNotifiedTimestamps[p] > 12 then
                                        ...
                                        if Config.HackerAutoLoad and Config.HackerProfile and Config.HackerProfile ~= "" then
                                            ...
                                            if ProfileSystem and ProfileSystem.current ~= targetProfile then
                                                ...
                                                if loaded then
                                                    ShowNotification("Reyex Hub", "Hacker detected! Loaded profile: " .. targetProfile, "SUCCESS", 3.5)
                                                end
                                            end
                                        end
                                    end
                    ...
                    if p ~= LocalPlayer and not modNotifiedCache[p] then
                        ...
                        if isMod then
                            ...
                            if Config.NotifyMods then
                                ShowNotification("Reyex Hub", "STAFF / MOD DETECTED: " .. p.DisplayName .. " (Rank: " .. tostring(rank or "Staff") .. ")", "ERROR", 5)
                            end
                        end
                    end
                end
...
setMenuVisible(true)
ShowNotification("Reyex Hub", "Loaded Reyex Hub successfully.", "SUCCESS", 4)
